  ...
  },
  {
    ...
    cell: row => {
      ...
      return (
        ...
        <>
          <Button.Ripple className='btn-icon' outline color="flat-primary" onClick={() => {
          
            history.push(`/edit_server?id=${row.hostid}`)
          }} >
        <Edit2 />
       {/* // {row.status ===  'Active' ? "InActive" : 'Active'} */}
      </Button.Ripple>
      <Button.Ripple className='btn-icon ml-0' color="flat-danger" onClick={() => {
  axios.delete(`${Apiurl}configuration/configwizards/servermonitor//${row.hostid}`).then(response => response.data).then(result => {
    console.log(result.message)
