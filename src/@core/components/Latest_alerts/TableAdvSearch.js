...
const DataTableAdvSearch = (props) => {
  ...
  const getAllUsers = () => {
    // setdata([{
    //   hostname: 'name', service: 'port', output: 'address', lastcheck: 'type', nextcheck: 'active', lastut: 'system',
    //   lastdt: 'system', statechange: '100%', severity: 'critical'
    // }])
    axios.get(`${Apiurl}incidentmanagement/latestalerts`).then(response => response.data).then(result => {
