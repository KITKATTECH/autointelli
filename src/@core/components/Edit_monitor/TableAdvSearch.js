...
const DataTableAdvSearch = ({id}) => {
  ...
  const getAllUsers = () => {
   // alert(id)
   // setdata([{servername:'name', port:'2.5',  address:'5',  type:'5',  status:'yes', system:'no'}])
  // if (props.data.id) {
      axios.get(`${Apiurl}configuration/configwizards/servermonitor/${id}`).then(response => response.data).then(result => {
  ...
  return (
    <Fragment>
      ...
      <Card>
        ...
        <CardBody>
        <Row as Card className='match-height'> 
                    <h2 className='content-header-title1 float-left'>Services </h2> 
</Row>
<Row className='mx-0 mt-1 mb-50'>
          ...
          <Col className='d-flex align-items-end justify-content-end mt-sm-0 mt-1' lg="12" sm='6'>
            ...
            <Button.Ripple  color='primary' className='ml-2  d-flex justify-content-end ' onClick={toggleSidebar}>
            Add Service
          </Button.Ripple>
          </Col>
...
{/* 
       ...
       <Sidebar open={sidebarOpen} toggleSidebar={toggleSidebar} isEdit={''} hostid={id}/>
    </Fragment>
  )
}
