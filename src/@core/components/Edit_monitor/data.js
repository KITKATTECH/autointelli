...
export const advSearchColumns = [
  ...
  {
    name: 'Frequency',
    selector: 'max_check_attempts',
    sortable: true,
    minWidth: '100'
  },
  {
    name: 'Retry Frequency',
    selector: 'retry_intervla',
    sortable: true,
    minWidth: '200px'
  },
  ...
  // },
  {
    ...
    cell: row => {
      ...
      return (
        ...
        <>
      <Sidebar open={sidebarOpen} toggleSidebar={toggleSidebar} isEdit={row.serviceid} hostid={localStorage.getItem('hostid')}/>

        <Button.Ripple className='btn-icon' outline color="flat-primary"onClick={toggleSidebar }>
          <Edit2/>
       
       {/* // {row.status ===  'Active' ? "InActive" : 'Active'} */}
      </Button.Ripple>
      <Button.Ripple className='btn-icon ml-0' color="flat-danger" onClick={() => {
  axios.delete(`${Apiurl}/configuration/configwizards/servermonitor/deleteservice/${row.serviceid}`).then(response => response.data).then(result => {
    console.log(result.message)
